...
# loading the data
sunset_data <- read.csv("C:/Users/jules/OneDrive/Desktop/Sunset-Prediction-Project/Sunset-Data/sunset_quality_data.csv")
...
knitr::include_graphics("C:/Users/jules/OneDrive/Desktop/Sunset-Prediction-Project/Images/roc_auc_curve.png")
```
Above is a visualization to help better understand a ROC curve. The area between the curve and the *Random Classifier* line is the AUC score, which increases as the curve approaches a *Perfect Classifier*. While the curve is a visualization of the ROC AUC score, the numerical AUC value between 0.5 and 1.0 is the determining metric of performance.
...
The ROC AUC scores seem to vary quite a lot depending on the number of trees, but overall it is clear that more trees leads to a higher ROC AUC score. The optimal node size was 5, present in the middle plot, with 10 trees, and 2 randomly selected predictors. Unless our next model can surpass this, the random forest seems to be the best model.
...
As we can see in our tibble, the Random Forest model performed the best overall with a ROC AUC score of 0.9988, with the support vector machine close behind at 0.9583. Of course, this is only fitted on the training data, so our model still needs to perform on our testing data that we have reserved for exactly this. We will be moving forward with this Random Forest model, but we will also explore the support vector machine model's performance on the testing data for the sake of exploration. Let's now find their true performances on our testing dataset.
...
Random Forest 249! The random forest model #249 seemed to have performed the best overall from all the random forest models. This is on top of being the best of the seven different prediction models. Below is the model's output and scores, as well as the its associated parameters. 
...
The computed Support Vector Machine model ROC AUC score on the testing dataset is as follows. As we can notice, it is about 0.02 higher than the random forest model, which is a very large margin of difference in the world of statistics and ROC AUC scores. 
```{r}
...
sunset_svm_roc_auc
```
With a final ROC AUC score of 0.7126, we can officially say that our support vector machine model performed better than the random forest model on our testing data.    
